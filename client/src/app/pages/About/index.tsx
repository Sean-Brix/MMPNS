...
import { FoundressSection } from './FoundressSection';
...
type AboutImageSlot = 'brandLogo' | 'sharedMariaPia' | 'aboutOrgChart';

const ABOUT_IMAGE_LABELS: Record<AboutImageSlot, string> = {
  brandLogo: 'About hero school seal',
  sharedMariaPia: 'Foundress section image',
  aboutOrgChart: 'Organizational structure chart',
};
...
export const About: React.FC = () => {
  ...
  const [editingSlot, setEditingSlot] = React.useState<AboutImageSlot | null>(null);
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (cloudUrl) {
        ...
        setUploadNotice('Image uploaded to cloud storage folder: principal-edits/about.');
        setEditingSlot(null);
        return;
      }
    } catch (error) {
  ...
  return (
    <div className="relative bg-[#fcfaf2] min-h-screen font-serif">
      {isImageEditMode && (
        <div className="fixed top-[88px] md:top-[122px] right-4 z-40 w-[min(90vw,320px)] rounded-xl bg-[#185C20] text-white border border-white/15 shadow-xl p-3 space-y-2">
          ...
          {(Object.keys(ABOUT_IMAGE_LABELS) as AboutImageSlot[]).map((slot) => (
            <button
              key={slot}
              type="button"
              onClick={() => {
            ...
            >
              <p className="text-[11px] font-semibold text-white/85">{ABOUT_IMAGE_LABELS[slot]}</p>
      ...
      <AboutHero schoolSeal={siteImageSlots.brandLogo} />
      <FoundressSection foundressImg={siteImageSlots.sharedMariaPia} isMobile={isMobile} />
      <MissionVision />
      <CoreValues />
      <OrgStructure orgChart={siteImageSlots.aboutOrgChart} />
      <Accreditation />
      <AdmissionCTA />
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div>
                <h3 className="text-lg font-bold text-[#185C20]">Edit About Page Image</h3>
                <p className="text-xs text-gray-500 mt-0.5">{ABOUT_IMAGE_LABELS[editingSlot]}</p>
              </div>
              <button
                type="button"
                onClick={closeModal}
            ...
            <div className="p-5 space-y-4">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-gray-200 overflow-hidden bg-gray-50">
                  ...
                  <img src={siteImageSlots[editingSlot]} alt={ABOUT_IMAGE_LABELS[editingSlot]} className="w-full h-52 object-contain bg-white" />
                </div>
                <div className="rounded-xl border border-gray-200 overflow-hidden bg-gray-50">
                  <p className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-gray-500 border-b border-gray-200 bg-white">
                    Default Preview
                  </p>
                  <img src={SITE_IMAGE_DEFAULTS[editingSlot]} alt={`Default ${ABOUT_IMAGE_LABELS[editingSlot]}`} className="w-full h-52 object-contain bg-white" />
                </div>
              </div>
